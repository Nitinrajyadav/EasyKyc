...
public class CommonUtils {
    ...
    public static final String PREF_USER_BHAMASHAH_ID = "pref_user_bhamashah_id";
    public static final String PREF_CAN_SHOW_QR = "pref_can_show_qr";
...
//        editor.putString(PREF_ACTIVE_USER, json);
    ...
    public static void setUserBhamashahId(String userId) {
        SharedPreferences.Editor settings = getSettings().edit();
        settings.putString(PREF_USER_BHAMASHAH_ID, userId);
        settings.apply();
    }

    public static String getPrefUserBhamashahId() {
        return getSettings().getString(PREF_USER_BHAMASHAH_ID, null);
    }


    public static void setPrefCanShowQr(boolean status) {
        getSettings().edit().putBoolean(PREF_CAN_SHOW_QR, status).apply();
    }

    public static boolean canShowQr() {
        return getSettings().getBoolean(PREF_CAN_SHOW_QR, false);
    }

    public static Bitmap getBitmapFromText(Context context, String Value) throws WriterException {
        BitMatrix bitMatrix;
        try {
            bitMatrix = new MultiFormatWriter().encode(Value, BarcodeFormat.QR_CODE, 250, 250, null);
        ...
        for (int y = 0; y < bitMatrixHeight; y++) {
            ...
            for (int x = 0; x < bitMatrixWidth; x++) {
                ...
                pixels[offset + x] = bitMatrix.get(x, y) ? ContextCompat.getColor(context, R.color.colorAccent) : ContextCompat.getColor(context, R.color.colorPrimary);
            }
        }
    ...
    public static SharedPreferences getSettings() {
        if (sharedPref == null) {
            sharedPref = KycApp.getInstance().getSharedPreferences(KycApp.PACKAGE_NAME, Context.MODE_PRIVATE);
        }
        return sharedPref;
    }
